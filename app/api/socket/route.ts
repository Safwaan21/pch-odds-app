...
// Store for active connections and game state (note: this will reset on cold starts in serverless)
let io: any = null;
...
export async function POST(request: NextRequest) {
  // If socket.io server is already running, return its status
  if (io) {
    return NextResponse.json({ 
      status: 'Socket server already running',
      players: players.length,
      spectators: spectators.length
    });
  }
  ...
  try {
    ...
    if (!socket || !server) {
      console.error('Socket.io initialization failed: socket or server not available');
    ...
    io.on('connection', (socket: any) => {
      ...
      socket.on('submitGuess', (data: any) => {
    ...
    return NextResponse.json({ error: 'Socket.io server initialization failed', details: error }, { status: 500 });
  }
}
